...
export default function StudentCertificateSettingsPage() {
  ...
  const [signatureRight, setSignatureRight] = useState<string>('Principal Signature');
    ...
    <div className="space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight flex items-center space-x-2">
            ...
            <span>Student Certificate Settings</span>
          </h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Configure institutional branding templates and signing definitions for student certificates</p>
        </div>

        <div>
          <Button
            onClick={() => router.push('/dashboard/certificates/student-cert')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-3xl p-6 shadow-sm space-y-4">
          <h3 className="font-bold text-slate-805 dark:text-slate-200 text-base border-b pb-2">Customization Panel</h3>
          
          <form onSubmit={handleSave} className="space-y-4 text-xs font-semibold">
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Border Frame Style</label>
              <select
                value={borderStyle}
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Right Signature Label (Principal)</label>
              <input
                type="text"
        ...
        <div className="lg:col-span-2 flex flex-col items-center space-y-4">
          <div className="flex items-center gap-1.5 self-start text-xs font-bold text-slate-400 uppercase tracking-wider">
            <Eye size={14} />
            <span>Live Layout Preview Mockup</span>
          </div>
          ...
          <div className="w-full aspect-[1.414/1] bg-white p-10 relative flex flex-col justify-between shadow-md border-slate-100 select-none font-serif text-slate-800 pointer-events-none"
            style={{
              ...
              borderWidth: borderStyle === 'none' ? '0px' : borderStyle === 'double' ? '12px' : '4px',
          ...
          >
            ...
            <div className="text-center space-y-1.5 border-b border-indigo-900/10 pb-2">
              ...
              <p className="text-[9px] uppercase font-sans font-bold tracking-widest text-slate-400">{headerSub}</p>
              <div className="h-[1px] bg-indigo-900/10 w-full my-0.5" />
              <h3 className="text-sm font-bold italic tracking-wide text-indigo-900/70 font-serif">Certificate of Achievement</h3>
            </div>

            {/* Certificate Content Body */}
            <div className="py-4 space-y-2.5 text-[11px] leading-relaxed text-center px-4">
              <p>This is to certify that a placeholder student has accomplished their institutional studies.</p>
              <p>He/She has completed all academic examinations and bears good character conduct.</p>
            </div>
            ...
            <div className="flex justify-between items-end pt-8 text-[10px] font-sans font-semibold">
              ...
              <div className="text-center space-y-1 w-1/3">
                <p className="text-[8px] text-slate-400 italic">Date of Issue: DD/MM/YYYY</p>
              </div>
              <div className="text-center space-y-1.5 w-1/3">
                <p className="border-t border-slate-300 pt-1 text-slate-650 font-bold">{signatureRight}</p>
              </div>
            </div>
          </div>
        </div>
