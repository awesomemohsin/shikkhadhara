...
export default function SalariesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex justify-center items-center h-48 text-slate-400">Loading payroll history...</div>
      ) : (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <table className="w-full">
            <thead className="bg-slate-50/50 border-b border-slate-100">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-550 uppercase tracking-wider">Employee</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-550 uppercase tracking-wider">Type</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-550 uppercase tracking-wider">Basic Salary</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-550 uppercase tracking-wider">Net Amount</th>
              ...
              {salaries.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-slate-400 text-sm">
                    No payroll entries found for tenant: <span className="font-semibold text-indigo-500">{tenantSlug}</span>
                  </td>
                </tr>
              ) : (
                salaries.map((salary) => (
                  <tr key={salary._id} className="hover:bg-slate-50/40">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                        salary.status === 'paid' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-amber-50 text-amber-700 border border-amber-100'
                      }`}>
                        {salary.status}
                      </span>
                    </td>
