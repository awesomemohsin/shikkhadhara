...
import { User, Calendar, BookOpen, DollarSign, Award, Clock, MapPin, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function StudentPortalPage() {
  ...
  const fetchStudentProfile = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const studentDoc = data.students?.[0];
        if (studentDoc) {
          setStudent(studentDoc);
          // Concurrently fetch routine, fees, and results using student info
          fetchRelatedData(studentDoc);
        } else {
          setLoading(false);
        }
      } else {
  ...
  const fetchRelatedData = async (studentDoc: any) => {
    try {
      const headers = { Authorization: `Bearer ${token}` };
      const [resRoutine, resFees, resResults] = await Promise.all([
        fetch(`/api/routines?class=${studentDoc.class}&section=${studentDoc.section}`, { headers }),
        fetch(`/api/fees?studentId=${studentDoc._id}`, { headers }),
        fetch(`/api/exam-results?studentId=${studentDoc._id}`, { headers }),
      ]);
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-[60vh] text-slate-400 font-medium">
        Loading student portal profile...
      </div>
    );
  }
  ...
  if (!student) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center p-6 bg-white rounded-2xl shadow-sm border border-red-50">
        ...
        <h2 className="text-xl font-bold text-slate-800 mb-2">Profile Unlinked</h2>
        <p className="text-slate-500 max-w-sm">
          Your user account is not linked to any student profile. Please contact the administrator.
        </p>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-r from-indigo-600 via-indigo-700 to-violet-700 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
          ...
          <div className="flex-1 text-center md:text-left space-y-2">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-3 text-sm text-indigo-100 font-medium">
              <span className="bg-white/10 px-2.5 py-1 rounded-lg">Class: {student.class}</span>
              <span className="bg-white/10 px-2.5 py-1 rounded-lg">Section: {student.section}</span>
              <span className="bg-white/10 px-2.5 py-1 rounded-lg">Roll No: {student.rollNumber || 'N/A'}</span>
            </div>
            <p className="text-xs text-indigo-200">Enrollment ID: {student.enrollmentId}</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-6">
          ...
          {activeTab === 'routine' && (
            <div className="space-y-6">
              ...
              {routines.length === 0 && (
                <div className="text-center py-12 text-slate-400 text-sm">No class routine scheduled yet.</div>
              )}
            </div>
          )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50/50 border-b border-slate-100">
                  <tr>
                    <th className="px-4 py-3 text-left font-semibold text-slate-500 uppercase tracking-wider text-xs">Billing Period</th>
                    <th className="px-4 py-3 text-left font-semibold text-slate-500 uppercase tracking-wider text-xs">Fee Description</th>
                    <th className="px-4 py-3 text-right font-semibold text-slate-500 uppercase tracking-wider text-xs">Total Dues</th>
                    <th className="px-4 py-3 text-right font-semibold text-slate-500 uppercase tracking-wider text-xs">Paid Amount</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {fees.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-4 py-8 text-center text-slate-400">No fee records found.</td>
                    </tr>
                  ) : (
                    fees.map((fee) => (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {results.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-4 py-8 text-center text-slate-400">No exam results recorded.</td>
                    </tr>
                  ) : (
                    results.map((res) => (
