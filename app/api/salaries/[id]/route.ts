...
export async function PUT(
  ...
  try {
    ...
    const salary = await TenantQuery.findOneAndUpdate(
      Salary,
      headerTenantId,
      { _id: id },
      data,
...
export async function DELETE(
  ...
  try {
    ...
    const id = params.id;
    const salary = await TenantQuery.findById(Salary, headerTenantId, id);
    if (!salary) {
      return NextResponse.json({ message: 'Salary record not found' }, { status: 404 });
    }

    await TenantQuery.deleteOne(Salary, headerTenantId, { _id: id });
