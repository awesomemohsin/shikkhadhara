...
export async function PUT(
  ...
  try {
    ...
    const hostel = await TenantQuery.findOneAndUpdate(
      Hostel,
      headerTenantId,
      { _id: id },
      data,
...
export async function DELETE(
  ...
  try {
    ...
    const id = params.id;
    const hostel = await TenantQuery.findById(Hostel, headerTenantId, id);
    if (!hostel) {
      return NextResponse.json({ message: 'Hostel not found' }, { status: 404 });
    }

    await TenantQuery.deleteOne(Hostel, headerTenantId, { _id: id });
