...
export async function GET(request: NextRequest) {
  try {
    ...
    // All authenticated users of this tenant can query hostel status
    if (!validateRequestAccess(role, userTenantId, Role.STUDENT, headerTenantId)) {
      return NextResponse.json({ message: 'Forbidden: Insufficient privileges' }, { status: 403 });
    }

    const hostels = await TenantQuery.find(Hostel, headerTenantId, {});
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only INSTITUTION_ADMIN or SUPER_ADMIN can configure hostels
    if (!validateRequestAccess(role, userTenantId, Role.INSTITUTION_ADMIN, headerTenantId)) {
      return NextResponse.json({ message: 'Forbidden: Insufficient privileges' }, { status: 403 });
    }

    const data = await request.json();
    const hostel = await TenantQuery.create(Hostel, headerTenantId, data);
