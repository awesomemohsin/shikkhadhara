...
export async function GET(request: NextRequest) {
  try {
    ...
    const lessons = await TenantQuery.find(LessonPlan, headerTenantId, {}, null, { sort: { class: 1, subject: 1 } });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!data.class || !data.subject || !data.topic) {
      return NextResponse.json({ message: 'Class, Subject, and Topic are required' }, { status: 400 });
    }

    const lesson = await TenantQuery.create(LessonPlan, headerTenantId, {
      class: data.class,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ message: 'Lesson Plan ID is required' }, { status: 400 });
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await TenantQuery.deleteOne(LessonPlan, headerTenantId, { _id: id });
