...
export async function GET(request: NextRequest) {
  try {
    ...
    // Gallery GET is public for anyone accessing the tenant workspace
    const gallery = await TenantQuery.find(Gallery, headerTenantId, {}, null, { sort: { createdAt: -1 } });
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only admins or super admins can post new photos/announcements to the gallery
    if (!validateRequestAccess(role, userTenantId, Role.INSTITUTION_ADMIN, headerTenantId)) {
      return NextResponse.json({ message: 'Forbidden: Insufficient privileges' }, { status: 403 });
    }

    const data = await request.json();
    const item = await TenantQuery.create(Gallery, headerTenantId, data);
