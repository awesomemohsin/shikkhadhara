...
export async function PUT(
  ...
  try {
    ...
    const id = params.id;
    const data = await request.json(); // e.g. status: 'closed'

    const ticket = await TenantQuery.findOneAndUpdate(
      SupportTicket,
      headerTenantId,
      { _id: id },
      data,
...
export async function DELETE(
  ...
  try {
    ...
    const id = params.id;
    const ticket = await TenantQuery.findById(SupportTicket, headerTenantId, id);
    if (!ticket) {
      return NextResponse.json({ message: 'Ticket not found' }, { status: 404 });
    }

    await TenantQuery.deleteOne(SupportTicket, headerTenantId, { _id: id });
